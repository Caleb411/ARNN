...
from main.power.config import get_param
...
if __name__ == '__main__':
    ...
    data = np.array(df)
    data = np.array(data, dtype='float')  # 数据删除时间列
    ...
    test_predict = lin_reg.predict(np.vstack(X).reshape(-1, get_param('lookback')//get_param('step')*get_param('dim'))) * std[0] + mean[0]
